...
3. Run `generate-diff-report.sh` to create an HTML report with selectable Git and locally rendered Difftastic JSON views, including inline and side-by-side layouts. Warn when `difft` is unavailable, but continue with Git diff.
