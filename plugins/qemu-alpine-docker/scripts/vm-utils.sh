...
acquire_single_vm_lock() {
    ...
    if mkdir "$ACTIVE_LOCK_DIR" 2>/dev/null; then
        ...
        echo "$$" > "${ACTIVE_LOCK_DIR}/launcher.pid"
        return 0
    fi
    local active_pid active_name
    ...
    if process_is_running "$active_pid"; then
        echo "Error: VM '${active_name}' already owns the global lock (PID ${active_pid})." >&2
        return 1
    fi
...
clear_vm_process_state() { rm -f "$(vm_pid_file)"; release_single_vm_lock; }
