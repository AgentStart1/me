...
- Keep Claude agent instructions in Markdown with only Claude-compatible frontmatter (`model` and `effort`). Keep Codex model and `model_reasoning_effort` metadata in the adjacent `agents/codex-routing.toml`; the installer promotes it to top-level fields in generated Codex TOML.
...
- Use `scripts/install-agents.sh` for manual installation into global or project-local `.codex/agents` and `.claude/agents` directories. It generates Codex `.toml` agents and copies Claude `.md` agents. Do not claim that installing a plugin automatically installs agents.
